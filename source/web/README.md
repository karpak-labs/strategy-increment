...
- The workflow includes public examples, two JSON imports, an explicit interval, facts-only mode or all four criteria, experiments derived from history, equity/drawdown paths, provenance, and evidence downloads. Nexus import controls appear only when that source is configured; Nexus is disabled in the Worker.
...
Clearing the browser's session cookie prevents access to the former session's private experiments. Download evidence packages for long-term retention.
