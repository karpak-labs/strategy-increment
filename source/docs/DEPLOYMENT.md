...
Edit the canonical files under `source/app/`, `source/web/`, and `source/worker.py`. Wrangler runs their generated copies, so changing source files requires a rebuild. Stop the running dev server, then run from `source/`:
...
Both code and assets update together with the embedded source fingerprint. The same local data and cookie secret are retained. Run `npm ci` again if the npm lockfile changes. Do not edit files inside the generated bundle or rely on source-file hot reload; rebuilding replaces generated application files. Use the same `--public-origin` override on every rebuild when developing at a nondefault address.
...
For an existing Worker with `SESSION_SECRET` already configured, run from a clean release checkout in `source/`. First deployments use the secret-upload variation below in place of the final deploy command.
...
For a first deployment with no stored runtime secret, prepare an owner-readable JSON file outside the repository containing a `SESSION_SECRET` value generated by a password manager (at least 32 random characters). Use its absolute path with the deploy command so code and secret are installed together:

```sh
uv run --locked pywrangler deploy --secrets-file /absolute/path/to/worker-secrets.json
```

Store that production secret securely and remove the temporary upload file after a successful deployment. Ordinary updates use `pywrangler deploy` without `--secrets-file`; Cloudflare preserves the existing encrypted secret. The generated `.dev.vars` is only for local development. These behaviors follow Cloudflare's [secrets deployment documentation](https://developers.cloudflare.com/workers/configuration/secrets/#upload-secrets-alongside-code).
